...
def test_text_qwen2_decoder_layer_decode(mesh_device):
    ...
    hf_model = (
        AutoModelForCausalLM.from_config(model_config, trust_remote_code=True)
...
def test_vision_dots_vision_block(mesh_device):
    ...
    DotsVisionBlock = get_class_from_dynamic_module(
        "modeling_dots_vision.DotsVisionBlock", "rednote-hilab/dots.ocr"
    )
    DotsVisionRMSNorm = get_class_from_dynamic_module(
        "modeling_dots_vision.RMSNorm", "rednote-hilab/dots.ocr"
    )
