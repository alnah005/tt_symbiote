...
def _has_run_on_devices_guard(fn) -> bool:
    ...
    for cell in closure:
        try:
            ...
            continue
        if isinstance(value, frozenset) and value and all(type(v).__name__ == "DeviceArch" for v in value):
            return True
    return False
...
def test_forward_methods_present_or_inherited():
    ...
    for cls in _REQUIRED_GUARDED_MODULES:
        ...
        # that we can inspect.
        if not src.strip():
            assert not _forward_is_own(cls), f"{cls.__name__}.forward was overridden but no source could be read"
