...
def _build_grid():
    ...
    for weight_dtype, fidelity, fp32_acc, packer_l1, act_dtype, mem_cfg in grid:
        cfg_id = f"w{weight_dtype}-mf{fidelity}-fp32{int(fp32_acc)}" f"-pkr{int(packer_l1)}-act{act_dtype}-{mem_cfg}"
        out.append(
            (
                cfg_id,
                {
...
def _make_sweep_class(cfg):
    ...
    class TTNNLinearSweep(TTNNLinear):
        ...
        def preprocess_weights_impl(self):
            self.tt_weight_host = preprocess_linear_weight(self.weight, dtype=weight_dtype, layout=ttnn.TILE_LAYOUT)
            self.tt_bias_host = None
            if self.bias is not None:
                self.tt_bias_host = preprocess_linear_bias(self.bias, dtype=weight_dtype, layout=ttnn.TILE_LAYOUT)
...
def test_zzz_sweep_summary():
    ...
    passing = [r for r in rows if r["status"] == "ok" and r["pcc"] == r["pcc"] and r["pcc"] >= _PCC_THRESHOLD]
    ...
    if summary["top5"]:
        ...
        for entry in summary["top5"]:
            print(f"    {entry['cfg_id']:60s}  " f"pcc={entry['pcc']:.5f}  ms={entry['forward_ms']:.2f}")
