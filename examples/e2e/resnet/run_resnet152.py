"""End-to-end smoke run for ``microsoft/resnet-152`` on a Tenstorrent device.

Sibling of :mod:`examples.e2e.resnet.run_resnet50`. ResNet-152 is the
deepest of the five canonical Microsoft variants, also using the
three-conv bottleneck block (:class:`ResNetBottleNeckLayer`). Stage 3
expands to 36 blocks (vs 23 in ResNet-101 and 6 in ResNet-50).

Structurally supported but not yet hardware-verified; flip
``hw_verified`` to ``True`` for ``microsoft/resnet-152`` in
:data:`RESNET_TTNN_TUNING` after a successful run here.

Usage::

    source .venv/bin/activate        # see scripts/bootstrap_venv.sh
    python examples/e2e/resnet/run_resnet152.py
"""
...
print(f"ResNet-152 top-1: class {predicted_class_idx} = {label!r}")
