"""End-to-end smoke run for ``microsoft/resnet-101`` on a Tenstorrent device.

Sibling of :mod:`examples.e2e.resnet.run_resnet50`. ResNet-101 uses the
three-conv bottleneck block (:class:`ResNetBottleNeckLayer`) — same as
ResNet-50 but with deeper per-stage repetition counts (notably stage 3
expands from 6 to 23 blocks).

Structurally supported but not yet hardware-verified; flip
``hw_verified`` to ``True`` for ``microsoft/resnet-101`` in
:data:`RESNET_TTNN_TUNING` after a successful run here.

Usage::

    source .venv/bin/activate        # see scripts/bootstrap_venv.sh
    python examples/e2e/resnet/run_resnet101.py
"""
...
print(f"ResNet-101 top-1: class {predicted_class_idx} = {label!r}")
