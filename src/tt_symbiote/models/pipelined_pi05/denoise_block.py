...
class TTNNPi05DenoiseExpertAttention(TTNNPi05GemmaAttention):
    ...
    def forward(
        ...
        k_t = self.tt_wqkv.shape[-2] // 32
        n_t = self.tt_wqkv.shape[-1] // 32
        m_t = s // 32
        _qkv_pc = _denoise_tuned_pcfg(m_t, k_t, n_t, _g.x, _g.y)
        if _qkv_pc is None:
            _qkv_pc = matmul_pcfg(m_t, k_t, n_t, _g.x, _g.y, in0_block_w=8)
        if _qkv_pc is not None:
            qkv = ttnn.linear(
                hidden_states, self.tt_wqkv, dtype=ttnn.bfloat8_b, memory_config=_L1,
                program_config=_qkv_pc, compute_kernel_config=_expert_ck,
            )
        else:
            _cg = ttnn.CoreGrid(y=1, x=_g.x)
            qkv = ttnn.linear(
                hidden_states, self.tt_wqkv, dtype=ttnn.bfloat8_b, memory_config=_L1,
                core_grid=_cg, compute_kernel_config=_expert_ck,
            )
        q, k, v = ttnn.experimental.nlp_create_qkv_heads(
            qkv, num_heads=self.num_heads, num_kv_heads=self.num_kv_heads,
            transpose_k_heads=False, memory_config=_L1,
        )
        ttnn.deallocate(qkv)
        ...
        _spc = _denoise_sdpa_pcfg(q.shape[-2], kv_seq, _g.x, _g.y)
        if _spc is not None:
            _sdpa_kwargs["program_config"] = _spc
        attn_out = ttnn.transformer.scaled_dot_product_attention(
            q, k, v, attn_mask=attention_mask, is_causal=False, scale=self.scale,
            compute_kernel_config=get_sdpa_compute_kernel_config(), **_sdpa_kwargs,
        )
        ttnn.deallocate(q)

        attn_out = ttnn.experimental.nlp_concat_heads(attn_out, memory_config=_L1)
        ok_t = self.tt_o.shape[-2] // 32
        on_t = self.tt_o.shape[-1] // 32
        _o_pc = _denoise_tuned_pcfg(m_t, ok_t, on_t, _g.x, _g.y)
        if _o_pc is None:
            _o_pc = matmul_pcfg(m_t, ok_t, on_t, _g.x, _g.y, in0_block_w=8)
        if _o_pc is not None:
            out = ttnn.linear(
                attn_out, self.tt_o, dtype=ttnn.bfloat16, memory_config=_L1,
                program_config=_o_pc, compute_kernel_config=_expert_ck,
            )
        else:
            _cg = ttnn.CoreGrid(y=1, x=_g.x)
            out = ttnn.linear(
