...
DEFAULT_REFERENCE_ROOT = os.environ.get(
    "PI05_REFERENCE_ROOT", "/home/ttuser/salnahari/pi0_5_ref"
)

__all__ = ["TTNNPi05Model", "from_checkpoint", "load_reference_pi05_model"]

_L1 = ttnn.L1_MEMORY_CONFIG


class TTNNPi05Model(TTNNModule):
    ...
    def from_torch(cls, ref_model, config: Optional[Pi0_5ModelConfig] = None) -> "TTNNPi05Model":
        ...
        new.suffix_embedding = TTNNPi05SuffixEmbedding.from_torch(
            ref_model.suffix_embedding, cfg.suffix_config
        )
    ...
    def _noise_to_device(self, noise_torch, ahp: int):
        ...
        return ttnn.from_torch(noise_torch.contiguous(), dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=self.device)
    ...
    def _suffix_phantom_mask(self, prefix_len: int, ah: int, ahp: int):
        ...
        return ttnn.from_torch(mask, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=self.device, memory_config=ttnn.DRAM_MEMORY_CONFIG)
    ...
    def sample_actions(
        ...
        img_embeds = [self.backbone.embed_image(im) for im in images]
        ...
        for i in range(n):
            ...
            # prefix_kv_cache=None: the expert reads its static KV buffers (filled
            # from the VLM stores above); the explicit cache arg is unused.
            v = self._denoise_forward(x_t, None, prefix_len, suffix_mask, block_mods, final_mod)
