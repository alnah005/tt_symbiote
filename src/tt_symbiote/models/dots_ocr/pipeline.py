...
class TTNNDotsOCRPipeline(StatelessTTNNModule):
    ...
    def from_hf_model(
        ...
        and assembles the pipeline. If ``hf_model`` is provided it is used
        directly (the recipe/Auto path already has the model loaded, so this
        avoids a redundant second weight load); otherwise the model is loaded
        from ``model_path`` with ``trust_remote_code=True``.
        """
        if hf_model is None:
