...
def _vision_matmul_program_config(device, m_dim: int, k_dim: int, n_dim: int):
    ...
    # The 2D-mcast factory requires per_core_M to divide m_tiles exactly
    # (num_blocks_y = m_tiles / per_core_M <= grid_y). When the device grid_y
    # does not divide m_tiles -- e.g. Blackhole's 10-row grid with vision
    # m_tiles=352 (11264/32) -- the tuned config previously bailed to the slow
    # auto-config (in0_block_w=1, ~1.7% FLOPs, the single biggest prefill cost
    # on Blackhole). Instead, fall to the largest row count <= grid_y that
    # divides m_tiles so the tuned config still applies (using fewer rows when
    # divisibility forces it; columns/grid_x are unaffected). On Wormhole
    # (grid_y=8, m_tiles=352) this is a no-op since 8 already divides 352.
    grid_y_eff = grid_y
    if m_tiles % grid_y != 0:
        grid_y_eff = max((d for d in range(1, grid_y + 1) if m_tiles % d == 0), default=1)
    ...
    pc = ttnn.MatmulMultiCoreReuseMultiCastProgramConfig(
        compute_with_storage_grid_size=(grid_x, grid_y_eff),
...
class TTNNDotsPatchMerger(TTNNModule):
    ...
    def forward(self, hidden_states: ttnn.Tensor) -> ttnn.Tensor:
        ...
        # Block-sharded merger MLP (Wormhole 8x8 grid). fc1 in0 is L1-interleaved
        # BF16; fc1 output is L1 BLOCK_SHARDED over the 8x8 grid ([M/8, N/8] =
        # [384, 768] shards); fc2 consumes the fc1 output shard with no reshard.
        # The helpers return None when the device grid is not 8x8 (e.g. Blackhole
        # P150) or when folded M is not divisible by the shard grid -- in that
        # case fall back to a grid-agnostic DRAM-interleaved matmul pair below.
        ...
        if bs_available:
            ...
            # Grid-agnostic fallback (Blackhole P150x4 etc.): DRAM-interleaved
            # matmuls, GELU fused on fc1. Functional on any compute grid; slower
            # than the tuned 8x8 block-sharded path but correct. The col-sharded
            # w2 still yields a width-sharded output matching text embeddings.
            hidden_states = ttnn.reshape(
