...
def _next_power_of_2(n: int, minimum=256) -> int:
    ...
    Inlined from the generic ``modules/decoder_layer.py`` helper in the
    tt-metal reference to avoid pulling in the full generic decoder layer
    for this single utility.
    """
    if n <= 1:
        return 1
    if n <= minimum:
...
class TTNNBailingRotaryEmbedding(StatelessTTNNModule):
    ...
    def set_output_tensors_config_impl(self, output_tensors):
        ...
        from tt_symbiote.core.module import set_distributed_tensor_config
